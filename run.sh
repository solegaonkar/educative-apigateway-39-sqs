...
# -----------------------------------------------------------------
# Invoke POST to send message to queue
# -----------------------------------------------------------------
curl --location --request POST $url --header 'Content-Type: application/json' --data-raw '{ "Hello": "World" }'

# -----------------------------------------------------------------
# Invoke GET to receive message from queue
# -----------------------------------------------------------------
curl --location --request GET $url | jq -r ".ReceiveMessageResponse.ReceiveMessageResult.messages[0].Body" | jq
